...
model_path = "checkpoints/qwen-vl-chat_lora-True_qlora-False-no-bbox-8k-vqa"
...
val_file = "dataset/val_json/qwen_with_bbox_val.json"
...
for entry in tqdm(data, total=len(data)):
    ...
    response, history = model.chat(tokenizer, score_question, history=history, **kwargs)
    print(history)

    qwen_answer.append(history)


with open("results/pred/qwen_no_bbox_val_pred.json", "w", encoding="utf-8") as file:
