...
class LLaVANeXTVideoDataCollator(BaseDataCollator):
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        images: List[PIL.Image.Image] = [
            x for instance in instances for x in instance["images"]
        ]
        if len(images) > 0:
            vision_inputs.update(
                **self.processor.image_processor(images, return_tensors="pt")
            )

        # videos
        videos: List[np.ndarray] = [
            x for instance in instances for x in instance["videos"]
        ]
        if len(videos) > 0:
            ...
            assert (
                len(set([x.shape[0] for x in videos])) == 1
            ), "All videos must have the same number of frames"
            vision_inputs.update(
                **self.processor.video_processor(videos, return_tensors="pt")
            )
        ...
        for system_prompt, cur_convs in zip(system_prompts, conversations):
            ...
            for i, text in enumerate(cur_convs):
                if i % 2 == 0:
                    ...
                    num_video_tokens = len(
                        [m.start() for m in re.finditer("<video>", text)]
                    )
            ...
            if self.mask_question_tokens:
                cur_assistant_masks = torch.tensor(
                    temp["assistant_masks"][:max_len], dtype=torch.bool
                ).unsqueeze(0)
                assert (
                    cur_labels.shape == cur_assistant_masks.shape
                ), "Label and mask shapes do not match"
                cur_labels = torch.where(
                    cur_assistant_masks, cur_labels, self.IGNORE_TOKEN_ID
                )

            assert (
                cur_input_ids.shape == cur_labels.shape
            ), "Input and label shapes do not match"
        ...
        # sanity check
        assert total_image_tokens == len(
            images
        ), "Number of image tokens does not match the number of images"
        assert total_video_tokens == len(
            videos
        ), "Number of video tokens does not match the number of videos"
