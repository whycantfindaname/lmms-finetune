...
parser.add_argument("--golden_file", type=str, default='results/gvlmiqa_bench/description/benchmark_2k.json')
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(desp_file), "preciseness")
    os.makedirs(save_dir, exist_ok=True)
    save_file = os.path.join(save_dir, f"{os.path.basename(desp_file).split('.json')[0]}_preciseness_scores.json")
    ...
    for idx_meta, (golden_item, desp_item) in enumerate(zip(golden_data[idx_meta_start:idx_meta_end], save_data[idx_meta_start:idx_meta_end])):
        ...
        assert os.path.basename(image) == golden_item['filename']
        if os.path.exists(image):
            img_path = image
        else:
            img_path = os.path.join(image_folder, image)
        model_desp = desp_item['answer']
        golden_desp = golden_item['assessment']

        # TODO:细化到distortion, other, in-context distortion, in-contecxt other，每个角度都算preciseness
        system_prompt = (
            "You are an expert in image quality assessment. Your task is to evaluate the preciseness of the MLLM description "
            + "based on the detailed low-level visual information derived directly from the reference (human-generated) "
            + "description. Focus on whether the MLLM description [MLLM DESC] precisely reflects the key low-level visual details "
            + "reflected in the reference description [GOLDEN DESC]. "
            + "Please rate score 2 for totally no controversial low-level description, 1 for less controversial low-level description than golden description, and 0 for more controversial low-level description than gloden description. "
        ...
        try:
            ...
            import re
            content = gpt4o(img_path=None, system_prompt=system_prompt, query=preciseness_query)
