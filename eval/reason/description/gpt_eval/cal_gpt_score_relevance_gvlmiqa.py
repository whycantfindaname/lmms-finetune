...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(desp_file), "relevance")
    os.makedirs(save_dir, exist_ok=True)
    save_file = os.path.join(save_dir, f"{os.path.basename(desp_file).split('.json')[0]}_relevance_scores.json")
    ...
    for idx_meta, (golden_item, desp_item) in enumerate(zip(golden_data[idx_meta_start:idx_meta_end], save_data[idx_meta_start:idx_meta_end])):
        ...
        system_prompt = (
            "You are an expert in image quality assessment. Your task is to evaluate the relevance of the MLLM description [MLLM DESC]"
            + "based on the detailed low-level visual information derived directly from the reference (human-generated) "
            + "description [GOLDEN DESC], which may include blur, noise, exposure, artifact, color, lighting, focus, composition, etc. "
        ...
        try:
            ...
            import re
            content = gpt4o(img_path=None, system_prompt=system_prompt, query=relevance_query)
