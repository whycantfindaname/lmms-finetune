...
if __name__ == "__main__":
    ...
    for idx_meta, (golden_item, desp_item) in enumerate(zip(golden_data[idx_meta_start:idx_meta_end], save_data[idx_meta_start:idx_meta_end])):
        ...
        system_prompt = (
            ...
            + "question-answer pairs [GOLDEN DESC], which may include blur, noise, exposure, artifact, color, lighting, focus, composition, etc. "
        ...
        try:
            assert num_save_data==len(save_data)
            import re
            content = gpt4o(img_path, system_prompt, relevance_query)
