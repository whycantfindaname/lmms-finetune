...
if __name__ == '__main__':
    # use CUDA_VISIBLE_DEVICES to select the GPU to use
    import argparse
    ...
    parser.add_argument('--save_path', type=str, default='results/gvlmiqa_bench/internvl2/internvl2_description_gvlmiqabench.json', required=True, help='path to save the predicted answers')
    ...
    parser.add_argument('--use_original_tokenizer', default=False, action='store_true', help='use the original tokenizer')
    ...
    if args.use_original_tokenizer:
    ...
    for image in tqdm(not_complete, total=len(not_complete)):
        ...
        pixel_values = load_image(os.path.join(image_folder, image), max_num=12).to(torch.bfloat16).to(model.device)
        # single-image single-round conversation (单图单轮对话)
        question = args.query
        response = model.chat(tokenizer, pixel_values, question, generation_config)
        print(response)
        save_data.append({'image': os.path.join(image_folder, image), 'answer': response})
