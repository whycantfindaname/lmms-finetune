...
parser.add_argument("--qa_file", type=str, default='data/meta_json/benchmark/test/test_dist_qa_assess_v1.json')
    ...
    "json_schema": {
        ...
        "properties": {
            ...
            "items": {
                ...
                "required": ["question", "answer", "concerns", "question_type"],
...
if __name__ == "__main__":
    ...
    for idx_meta, meta_item in enumerate(meta_data[idx_meta_start:idx_meta_end]):
        ...
        for dist_item in dist_assess:
            ...
            dist_assess = dist_item['assessment']
            assess_query += f'{dist_name}: {dist_assess}\n'
        print(assess_query)
        
        # print(query)
        # system_prompt_meta_data =(
        #     " You are an expert in image quality assessment. Generate multiple yes-or-no/what/how/why/where question and answer pairs based on the following image information and distortion information."
        ...
        #     + " The distortion information includes the distortion id, distortion type, position, severity, region quality, region importance, visual manifestation and perception impact."
        ...
        #     + " 3.Avoid generating questions that are essentially the same.\n"
        #     + " 4.You need to generate questions for each distortion type in the information, and each question answer pair should involve as more concerns as possible. The more concerns addressed in each question, the more informative the question will be."
        #     + " 5.For each distortion type, generate as few questions as possible while ensuring that all relevant information is covered."
        #     + " 6.The combinations of concerns addressed in each question need to be arranged flexibly to avoid repetition or overly fixed patterns."
        #     + " 7.Your answers should be accurate, if possible, give your reasoning.\n"
        ...
        system_prompt_assess_data = (
            "You are an expert in image quality assessment. Generate multiple yes-or-no/what/how/why/where question and answer pairs based on the following single distortion assessment provided to you."
            + " Our goal is to evaluate the AI assistant's accuracy in assessing image quality and each type of distortion, so the questions you generate should comprehensively evaluate its capabilities." 
            + " These questions should be distortion-oriented, and for each distortion type, 8 concerns need to be considered:\n"
            + " 1. the existence of the distortion;\n"
            + " 2. the type of the distortion;\n"
        ...
        try:
            ...
            # content = gpt4o(meta_query, system_prompt_meta_data)
