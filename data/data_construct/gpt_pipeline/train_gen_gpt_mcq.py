...
parser.add_argument("--mcq_file", type=str, default='data/meta_json/train/test/test_mcq_v1.json')
        ...
        "json_schema": {
            ...
            "properties": {
                ...
                "items": {
                    ...
                    "required": ["question", "answer", "false candidates", 'question_type'],
                    "additionalProperties": False
                }
                }
            },
    ...
    for idx_meta, meta_item in enumerate(meta_data[idx_meta_start:]):
        ...
        try:
            ...
            system_prompt_assess_data = (
                ...
                + " 3. When generating questions, pretend that you have only seen the image and have no prior knowledge of the provided information. So the questions should be clear, general, and easy to understand, without being overly specific."
                + " 4. Avoid generating questions that are essentially the same or too similar.\n"
                ...
                + " 7. Ensure that the grammar is correct and that the question-answer pairs are logically consistent and well-structured.\n"
                + " 8. The total number of generated questions should be less than 15.\n"
            ...
            system_prompt_assess_data = (
                "You are an expert in image quality assessment. Generate multiple what/how/why/where question and answer pairs based on the following overall image quality assessment provided to you."
                + " Our goal is to evaluate the AI assistant's accuracy in assessing image quality, so the questions you generate should comprehensively test its capabilities."
                + " When generating questions, pretend that you have only seen the image and have no prior knowledge of the provided information. So the questions must not reference the information provided directly, but rather focus on the image itself."
                + " Questions should be clear, general, and easy to understand, without being overly specific."
                + " The answer should be concise and only contain the core information with minimum words. "
                + " You should also generate several false answers for each question under the key of false candidates, which are also reasonable given the question by contradicting with the description."
