...
class QwenScorer(nn.Module):
    ...
    def forward(self, image_path: List[str], temperature=1, sys_prompt: str = "You are an expert in image quality assessment. Your task is to evaluate the quality of an image." ,bbox_list=None, dist_list=None):
        ...
        with torch.inference_mode():
            ...
            for prompt, path in tqdm(zip(prompts, image_path), total=len(prompts)):
                ...
                cal_logit = logit[:, -1, self.cal_ids_].half() / temperature
                pred_mos = float(self.cal_pred_mos(cal_logit))
                pred_mos_values.append(pred_mos)
                logits_dict = defaultdict(
                    float, {level: val for level, val in zip(self.level, output_logit)}
                )
    ...
    def cal_pred_mos(self, cal_logit):
        weight = torch.tensor(0.8, device=self.device)
        weighted_sum = torch.softmax(cal_logit, -1) @ self.weight_tensor
        max_indices = torch.argmax(cal_logit, dim=-1)
        if max_indices == 0:
            return torch.tensor(4.2, device=self.device) + weight * weighted_sum
        elif max_indices == 1:
            return torch.tensor(3.4, device=self.device) + weight * weighted_sum
        elif max_indices == 2:
            return torch.tensor(2.6, device=self.device) + weight * weighted_sum
        elif max_indices == 3:
